...
```javascript
// Supports ES6
// import { create, Whatsapp } from 'whatsapp_elmenoufi';
...
function start(client) {
  client.onMessage((message) => {
    if (message.body === 'Hi' && message.isGroupMsg === false) {
      client
        .sendText(message.from, 'Welcome whatsapp_elmenoufi 🕷')
        .then((result) => {
...
##### After executing `create()` function, **whatsapp_elmenoufi** will create an instance of whatsapp web. If you are not logged in, it will print a QR code in the terminal. Scan it with your phone and you are ready to go!

##### whatsapp_elmenoufi will remember the session so there is no need to authenticate everytime.
...
venom
  .create(
    ...
    // options
    {
      ...
      mkdirFolderToken: '', //folder directory tokens, just inside the whatsapp_elmenoufi folder, example:  { mkdirFolderToken: '/node_modules', } //will save the tokens folder in the node_modules directory
      ...
      autoClose: 60000, // Automatically closes the whatsapp_elmenoufi only when scanning the QR code (default 60 seconds, if you want to turn it off, assign 0 or false)
...
// Send basic text
await client
  .sendText('000000000000@c.us', '👋 Hello from whatsapp_elmenoufi!')
  .then((result) => {
    console.log('Result: ', result); //return object success
  })
  .catch((erro) => {
...
// Send file (whatsapp_elmenoufi will take care of mime types, just need the path)
...
await client.setProfileName('whatsapp_elmenoufi bot');
...
There are some tricks for a better usage of whatsapp_elmenoufi.
...
Building whatsapp_elmenoufi is really simple altough it contains 3 main projects inside
...
3. whatsapp_elmenoufi

```bash
> npm run build:whatsapp_elmenoufi
```
